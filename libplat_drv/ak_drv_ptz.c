...
static int degree2step(struct ak_motor *motor, int degree)
{
	int ret;
	ret = motor->cycle_step * degree / 360;
	return ret;
}
static int step2degree(struct ak_motor *motor, int steps)
{
	int ret;
	ret = steps * 360 / motor->cycle_step;
	return ret;
}
...
static void get_motor_param(struct ak_motor *motor)
{
	...
	ak_print_normal_ex ("Motor Background Thread ID %08x\r\n", (unsigned int)ak_thread_get_tid());
	ak_print_normal_ex ("Full Distanse Step %d/%d\r\n", motor->fulldst_step, motor->cycle_step);
	...
	if (!motor_turn(motor, steps, ANTICLKWISE)) {
		ak_print_normal_ex("Turning counter-clockwise\n");
		rg = steps;
		//ak_print_normal_ex("rg=%d\n", rg);
		motor_wait_hit_stop(motor, &rg, motor->hit_speed);
		//ak_print_normal_ex("rg=%d\n", rg);
	...
	if (!motor_turn_dg(motor, rg, CLKWISE)) {
		ak_print_normal_ex("Turning clockwise\n");
		rg = degree2step(motor, rg);
		ak_print_normal_ex("rg=%d\n", rg);
		motor_wait_hit_stop(motor, &rg, motor->hit_speed);
	}
...
static void* motor_turn_thread(void* data)
{
	...
	while(motor->run_flag){
		...
		if( 0 == motor->run_flag )
			break;
		ak_print_normal_ex("clkwise=%d, angle=%d\n",
...
static int check_dg (struct ak_motor *motor, int degree)
{
	...
	ak_print_normal_ex("Checking degrees and direction. Received %d degrees\r\n", degree);
	int dg = 0;
	...
	ak_print_normal_ex ("Move from %d to %d Step %d Degree %d\r\n", cur_step, set_step, step, dg);
...
int ak_drv_ptz_turn_to_pos(int h_degree, int v_degree)
{
	...
	ak_print_normal_ex ("Turn to (%d, %d)\r\n", h_degree, v_degree);
	ak_print_debug_ex ("Turn to (%d, %d)\r\n", h_degree, v_degree);
	if( 0 == check_dg(&akmotor[0], h_degree)) {
		ak_print_normal_ex ("Check complete. direction and dg are (%d, %d)\r\n", akmotor[0].cmd_data.clkwise, akmotor[0].cmd_data.dg);
